...
def invoke_agent(agent_id, agent_alias_id, session_id, prompt):
    try:
        client = boto3.session.Session().client(service_name="bedrock-agent-runtime", region_name='us-west-2')               #1 Add Region Name
        ...
        response = client.invoke_agent(
            agentId='QJELMFH20A',                     #2 Add Agent ID
            agentAliasId='YPTNUYUPI1',                #3 Add Agent Alias ID
            enableTrace=True,
